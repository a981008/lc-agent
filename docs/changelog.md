...
| v0.39 | **题解复制改为当前页签 + 关闭按钮固定**：「⧉ 复制全部」改为「⧉ 复制代码」——只复制当前激活页签的语言代码（纯代码原文，切页签跟随）；题解弹窗右上角 ✕ 改为 `sticky` 钉住——长题解滚动时关闭按钮始终可见，不再随内容滚走 |
| v0.40 | **杂项修复**：顶栏版本徽章显示 `vv0.x` 的双 v 笔误（模板手写 v 与数据自带 v 前缀重复）；README 增加 Gitee/GitHub 仓库地址与克隆命令 |
| v0.41 | **题目列表去掉无用「#」空列**（历史遗留占位，从未填过内容） |
| v0.42 | **去掉重复的「当前题目」展示**：Pipeline 面板标题改为「Pipeline」，当前题目信息只保留面板内带 slug 的动态行；且该行现随服务端状态实时同步（页面刷新后不再残留旧题） |
| v0.43 | **AI 解题过程实时展示**：LLM 调用接入 SSE 流式（anthropic/openai 双协议），生成与多语言翻译的增量输出经 WS `pipeline_step(delta)` 事件推送到面板「🤖 AI 解题过程」区——实时看到 AI 思考与写码过程，自动滚底，阶段切换自动分隔；增量按 400ms/800 字符合帧防 WS 洪泛，流式失败不影响判题主链路 |
| v0.44 | **手机端 Web 适配**：≤960px 切单列布局（控制面板上、流水线+历史下）、顶栏换行、检索栏纵向堆叠、表格横向滚动+单元格截断、弹窗贴底全宽（92dvh）、日志/过程区高度收敛；输入控件统一 16px 防 iOS 聚焦缩放、按钮触控目标加大；≤480px 超窄屏进一步收紧；桌面端双列布局不受影响（CDP 375/1440 双端回归） |
| v0.45 | **一题多解（AI 自主定数量）**：主解法 AC 后由 AI 规划该题还值得几种「思路本质不同」的解法（简单题可为 0，经典题 2-3 种），逐个生成 → 本地沙盒验证 → 真实提交，仅保留 AC 的进题解「## 多解法」小节；标识符折叠去重防「变量改名式」伪多解；失败静默丢弃不阻塞主链路；移除用户侧「每题补充解法数」设置 |
| v0.46 | 一题多解改造为 AI 自主规划数量（v0.45 行内容已含此改造；本行为版本号占位说明，代码见 be00b8e） |
| v0.47 | **多解法配套题解**：每个补充解法由 LLM 生成专属讲解（解题思路 / 关键实现细节 / 复杂度 LaTeX），与 AC 代码一并进「多解法」小节；讲解禁用标题输出并在拼接时剥除 `#` 行防层级混乱，LLM 失败回退一句话简述 |
| v0.48 | **日志分页渲染**：日志终端按「尾部窗口」分页（默认最新 150 条），「↑ 加载更早」逐页扩窗（每次 +150）、「↓ 回到最新」一键收回；prepend 保持视口不跳动，跟随模式新日志自动滚底；DOM 常驻节点恒 ≤ 窗口大小，长跑不再无限变大；移动端分页条自适应 |
| v0.49 | 题目列表移除「尝试」次数列（信息冗余，尝试历史仍可经「尝试记录」查看） |
| v0.50 | **每日一题自动完成**：启动后 90s 首查、之后每 20min 轮询官方 todayRecord 接口；识别当日每日一题并自动触发完整流水线（生成→沙盒→提交→多解法→翻译→归档）；已 AC 直接标记完成、付费题跳过、配额不足/忙时延后重试、触发即落 `daily:done:<日期>` 标记防重复；面板参数页可开关；新增 `POST /api/admin/daily-challenge` 手动补跑端点（实测当日题全自动 AC） |
| v0.51 | **翻译 TLE 正确解保留标注**：翻译到其他语言（如 Python）若自愈后仍 TLE——解法本身正确、仅超时限——不再丢弃：代码进「AC 代码」页签区并以「XX（TLE）」标注（排在已 AC 语言后），前端高亮按剥后缀匹配；摘要记「正确但 TLE（需该语言下更优实现）」；翻译提示词增加目标语言性能特征提醒（惯用法/渐进更优实现）降低 TLE 概率 |
| v0.52 | **会员题收录/标注/筛选**：题库继续收录会员题（现有 531 题）；题目列表题名旁金色「会员」徽章（状态徽章去掉重复 🔒）；检索栏新增「会员题」筛选（全部 / 仅会员题 / 仅免费题），服务端 `paid` 参数过滤；非会员保护保持：调度队列与手动触发均硬编码跳过付费题（「跑这题」按钮禁用） |
| v0.53 | 「AI 解题过程」面板不再显示多语言翻译的流式内容（仅生成阶段实时流；翻译只更新阶段状态灯，完成后汇报结果） |
...
| v0.58 | **多解法页签化 + 已解答可重跑**：① 题解「多解法」小节改为页签组件（```alt-tabs 块，每个解法一个页签：讲解 markdown + 高亮代码，点击切换），不再与正文混排；② 已解答题目可在列表「↻ 重跑」（付费题仍禁用），重跑后新题解覆盖旧题解（solutions 表 upsert + 文件覆盖 + Git 幂等提交） |
